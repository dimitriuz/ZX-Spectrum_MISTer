...
| Ports | Kempston joystick (#FFFD), standard ULA ports; **#FF reads the ULA attribute byte** (floating bus) unless the Beta claims it; #FE uses selective decode (A4,A3,A1,A0) — see limitations |
...
5. **#1FFD register**: new `reg [7:0] scorp_1ffd`. Write decode: `scorp_1ffd_wr = scorp & ~addr[15] & ~addr[1] & addr[12] & ~addr[13] & ~addr[14]` (#1FFD), latching `cpu_dout` on the io_wr edge. Cleared to 0 on reset. Port read conformance (speccy-bootcamp): #7FFD is **write-only** (no mux arm — reads fall through to the ULA port like other unattached ports); #1FFD reads return **#FF** on non-Turbo boards (this core models the base ZS-256, no Turbo), so `cpu_din` has one Scorpion arm: `(scorp & addr[14:0]==15'h1FFD) ? 8'hFF`. Shadow Monitor exit is a #1FFD *write* (=0), not a read.
   A second arm covers **port #FF**: it is the ULA's attribute port, not an unattached
   port. Fuse wires it that way for this machine — `scorpion_reset()` installs
   `machines_periph_pentagon()`, whose table holds
   `{ 0x00ff, 0x00ff, pentagon_select_ff_read, beta_sp_write }`, and that handler reads
   the Beta status port first and falls back to `spectrum_unattached_port()`, the
   floating bus. Unreal's `PRESET.SCORPION` agrees (`PortFF=1`). So `cpu_din` gets
   `(scorp & (addr[7:0] == 8'hFF)) ? port_ff` *ahead* of the catch-all, while every
   other unattached port keeps reading `#FF`. `fdd_sel` already claims #FF whenever the
   Beta is paged, which is the same precedence Fuse has. The ULA's `tmx_avail` is
   gated with `~scorp` at the instantiation (not on the wire — it also drives the
   SAA1099 chip select): a Scorpion has no Timex, and letting an `OUT (#FF)` latch
   `tmx_using_ff` would make `port_ff` return the Timex config instead of the
   attribute byte.
...
- **New `scorp_tim` input.** It gates two things, below. The raster itself is
  untouched: the Scorpion ULA *is* the stock ULA-48 raster (224T lines, 312-line
...
Built with Quartus 17.0.2 (Cyclone V, 5CSEBA6U23I7) and run on a DE10-Nano
against a real bootable TR-DOS disk. Everything below passes on the build in
`releases/`; steps 1-8 are the checklist worth repeating after any change here.
...
All five boot-menu entries work, as does the Shadow Service Monitor's Disk
utility (full-surface read of a 640 K image: 2560 sectors, no errors). The
Kempston joystick still answers `#1F` from BASIC - section 6, bug 2, is why that
is worth re-checking after any change to the port decode.
...
fixed, and both were first reproduced in an instrumented Fuse 1.7 - the reference
implementation for this machine - before the RTL was touched.

### Bug 1 - the Beta ROM was never paged out in `#8000-#BFFF`

Adding the `current_rom != 0` gate to the existing `if(addr[15:14])` page-out
has a trap in it, and we fell into it:

```systemverilog
if(addr[15:14] & (~scorp | scorp_cur_rom)) trdos_en <= 0;    // silently wrong
if((|addr[15:14]) & (~scorp | scorp_cur_rom)) trdos_en <= 0; // correct
```

A bitwise `&` extends its operands to the wider one and zero-fills the 1-bit
side, so `addr[15:14] & <1 bit>` is `{1'b0, addr[14] & cond}` - the `addr[15]`
term is silently dropped and TR-DOS is never paged out anywhere in
...
of the run) - so the file browser runs with the TR-DOS ROM still over
`#0000-#3FFF`. The same expression governs every machine, so getting it wrong
takes 48 TR-DOS and the Pentagon down with the Scorpion.
...
about 97,000 times. The two callers are separated by `#1FFD[1]`: only the Shadow
Monitor runs with its own ROM2 paged in, so that is the one case where the Beta
ports win and
...
- `page_write` for the Scorpion is `~A15 & ~A1` with no A14 term, so it is wider
  ...
  difference - checked by logging every `IN`/`OUT` of a full reference run.

## 7. How the two bugs in section 6 were found

Neither the trace capture used for the measurements above nor the Fuse harness is
part of this change; both live on the development branch (`scorpion-zs256`). For
reference:

- An **instrumented Fuse 1.7** that boots the same ROM set and disk headlessly,
  drives the boot menu from a key schedule and logs every paging event with the
  PC that caused it - and, crucially, can be switched over to our RTL's rules one
  at a time, so a hypothesis about the core is confirmed against a working
  machine in seconds. Both bugs were reproduced there before the RTL was touched,
  and both fixes were re-checked there before the core was rebuilt.
- An **OSD-gated in-core trace**: a border view of `{trdos_en, #1FFD[1],
  #7FFD[4]}` and 16 read-only registers at `#7AF0-#7AFF` capturing the machine
  state at the first `#3Dxx` fetch plus FDC activity. `#7AFx` is Turbo+/GMX
  territory the base ZS-256 ROM never touches, confirmed by disassembling pages
  0 and 2 - though note that a *write* there lands on `#7FFD` (A15=0, A14=1,
  A1=0), so the port is read-only in practice.
- The machine's own **Shadow Service Monitor**, which F11 reaches even from a
  hung CPU and which dumps PC, SP, every register pair and the paged banks. That
  single screen is what identified the `#0237` spin.
